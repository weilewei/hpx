...
namespace hpx { namespace threads { namespace policies
{
    ...
    {
        nothing_special = 0x0000,            ///< As the name suggests, this option
            ///< can be used to disable all other options.

        do_background_work = 0x0001,       ///< The scheduler will periodically
        ...
        reduce_thread_priority = 0x0002,  ///< The kernel priority of the
            ///< os-thread driving the scheduler will be reduced below normal.
        delay_exit = 0x0004,              ///< The scheduler will wait for some
        ...
        fast_idle_mode = 0x0008,          ///< Some schedulers have the capability
        ...
        enable_elasticity = 0x0010,        ///< This option allows for the
        ...
        enable_stealing = 0x0020,          ///< This option allows schedulers
            ///< that support work thread/stealing to enable/disable it
        enable_stealing_numa = 0x0040,     ///< This option allows schedulers
            ///<that support it to disallow stealing between numa domains
        assign_work_round_robin   = 0x0080,///< This option tells schedulers
            ///<that support it to add tasks round robin to queues on each core
        assign_work_thread_parent = 0x0100,///< This option tells schedulers
            ///< that support it to add tasks round to the same core/queue that the
            ///< parent task is running on
        steal_high_priority_first = 0x0200,///< This option tells schedulers
        ...
        steal_after_local         = 0x0400,///< This option tells schedulers
            ///< that support it to steal tasks only when their local
            ///< queues are empty
        enable_idle_backoff       = 0x0800,     ///< This option allows for certain
            ///< schedulers to explicitly disable exponential idle-back off
        enable_user_polling       = 0x1000,     ///< Whe enabled, this option
            /// tells the scheduler to call a user supplied function on each iteration
            /// (after execution of each task on each worker thread)
        default_mode =
                do_background_work |
